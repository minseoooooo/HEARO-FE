...
import { fetchApi } from "@/lib/api" // fetchApi 임포트

// ... (zod 스키마 및 타입 정의는 이전과 동일)
...
export default function AuthPage() {
    ...
    const handleCheckEmail = async () => {
      ...
      try {
        ...
        const response = await fetchApi(`https://api.herehear.p-e.kr/api/v1/auth/check-email?email=${email}`)
    ...
    // ... (handleEmailInputChange, toggleForm 함수는 이전과 동일)
    ...
    const onSubmit = async (values: z.infer<typeof loginSchema | typeof signUpSchema>) => {
      ...
      if (isLogin) {
        try {
          // fetch 대신 fetchApi 사용
          const response = await fetchApi("https://api.herehear.p-e.kr/api/v1/auth/login", {
            method: "POST",
        ...
        try {
          ...
          // fetch 대신 fetchApi 사용
          const response = await fetchApi("https://api.herehear.p-e.kr/api/v1/auth/signup", {
              method: "POST",
              body: JSON.stringify({
    ...
    // ... (return JSX 부분은 이전과 거의 동일, Input의 onChange만 수정)
    return (
